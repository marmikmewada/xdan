...
export default function EditLocationPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            router.push(`/admin/locations/${id}`); // Redirect to the store's location page
        } catch (err) {
            setError(err.message);
        } finally {
            setIsLoading(false);
        ...
        <div className="container mx-auto px-4 py-8">
            ...
            <form onSubmit={handleSubmit} className="max-w-2xl">
                        ...
                        {availableSlots.length === 0 ? (
                            ...
                            availableSlots.map((slot, index) => (
                                ...
                                >
                                    ...
                                    {slot.status === 'booked' && <div className="text-sm text-gray-500">Booked by: {slot.bookedBy}</div>}
                                </button>
                            ))
                        )}
                    </div>
                ...
                <div className="mb-4">
                    ...
                    >
                        {isLoading ? 'Booking...' : 'Confirm unavailable'}
                    </button>
                </div>
            </form>
        </div>
