    ...
    <main className={`min-h-screen ${bgColor} ${textColor}`}>
      ...
      <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <section className={`py-12 ${bgColor}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {stores.map((store) => (
              <div key={store._id} className={`${cardBg} shadow-md transition-all duration-300 hover:shadow-lg relative group`}>
                <Link href={`/locations/${store?._id}`}>
                  <div className="relative h-48 w-full">
                    {store.coordinates ? (
                      ...
                      <div className="flex items-center justify-center h-full bg-gray-200 text-gray-500">No location</div>
                    )}
                  </div>
                  ...
                  <div className="p-4">
                    ...
                    <Link href={`/locations/${store._id}`} className="text-blue-500">View Store</Link>
                  </div>
                </Link>
              </div>
            ))}
