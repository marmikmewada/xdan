  ...
  return (
    <div className={`min-h-screen ${containerClass}`}>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className={`shadow-lg rounded-lg overflow-hidden ${cartBackgroundClass}`}>
            <div className="divide-y divide-gray-200">
              {cartItems.map((item, index) => (
                <div
                  key={index}
                  className={`py-4 px-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 justify-between items-center ${selectedMode === 'dark' ? 'text-white' : 'text-black'}`}
                >
                  ...
                  <div className="flex justify-end space-x-3 mt-4 sm:mt-0">
                    <button
                      onClick={() => handleDecreaseQuantity(item.itemId)}
                      className="text-lg font-semibold text-gray-600 bg-gray-200 px-3 py-2 rounded hover:bg-gray-300 transition duration-300"
                    >
                      -
                    </button>
                    <button
                    ...
                    <button
                      ...
                      className="text-lg font-semibold text-black bg-white p-3 rounded-full hover:bg-gray-300 transition duration-300"
                    >
                      <FaTrashAlt className="w-5 h-5" />
                    </button>
                  </div>
                  <div className="text-lg font-semibold text-orange-500">£{item.price * item.quantity}</div>
                </div>
              ))}
            </div>

            <div className="py-4 px-6 text-right">
              <p className={`text-xl font-semibold ${totalTextColorClass}`}>Total: £{cartTotal}</p>
              <button
                onClick={handleEmptyCart}
                className={`mt-3 px-6 py-3 bg-gradient-to-b ${selectedMode === 'dark' ? 'bg-gradient-to-r from-gray-800 to-black text-white' : 'bg-black text-black'} text-lg font-semibold hover:bg-red-800 transition-all`}
              >
                Empty Cart
              </button>
            </div>
        ...
        {cartItems.length > 0 && (
          <div className="mt-6 flex justify-center sticky bottom-0 w-full py-4">
            <button
              onClick={() => router.push('/checkout')}
              className={`px-8 py-3 ${buttonClass} text-lg font-bold shadow-md transition-all duration-300 hover:scale-105 active:scale-95`}
            >
              Proceed to Checkout
            </button>
          </div>
