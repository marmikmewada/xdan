    ...
    return (
        <div className={`min-h-screen ${bgColor} ${textColor} transition-colors duration-300`}>
            ...
            <div className="container mx-auto px-4 py-8">
                <h1 className="text-2xl md:text-3xl font-bold mb-6">Select a Bed for {date}</h1>

                {notification && (
                    <div className={`fixed top-4 left-1/2 transform -translate-x-1/2 z-50 px-4 py-2 shadow-lg ${
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {availableSlots.length === 0 ? (
                        ...
                        availableSlots.map((bed) => (
                            ...
                            <div
                                key={bed._id}
                                className={`${buttonBgColor} shadow-lg overflow-hidden cursor-pointer transition-all duration-300`}
                                onClick={() => handleBedClick(bed._id)}
                            >
                                <div className="relative h-48">
                                    {bed.imageUrl && bed.imageUrl.length > 0 ? (
                                        <Image
                                            src={bed.imageUrl[0]}
                                            alt={bed.bedName}
                                            layout="fill"
                                ...
                                <div className="p-4">
                                    <h3 className="font-semibold text-lg mb-2 text-white">{bed.bedName}</h3>
                                    <p className="text-sm text-gray-300 mb-2">Date: {date}</p>
                                    {bed.storeRef && bed.storeRef.length > 0 && (
                                        <p className="text-sm text-gray-300">
                                            Store: {bed.storeRef[0].name}
                                        </p>
                                    )}
                                </div>
                            </div>
